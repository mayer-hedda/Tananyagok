<?php
require_once('MembershipFunctionType.php');
error_reporting(5);
...
*/
	...
	if ($P==$this->FMiddle) return 1;
	 ...
	 if ($this->FType==MemberShipFunctionType::LINFINITY->value) {
	    if ($P<=$this->FMiddle) return 1;
		if (($P>$this->FMiddle) AND ($P<$this->FB)) return ($this->FB-$P)/($this->FB-$this->FMiddle);
	 }
	
	 if ($this->FType==MemberShipFunctionType::RINFINITY->value) {
	    if ($P>=$this->FMiddle) return 1;
		if (($P<$this->FMiddle) AND ($P>$this->FA)) return ($P-$this->FA)/($this->FMiddle-$this->FA);
	 }

	if ($this->FType==MemberShipFunctionType::TRIANGLE->value) {
	   if(($P<$this->FMiddle) AND ($P>$this->FA)) return ($P-$this->FA)/($this->FMiddle-$this->FA);
	   if(($P>$this->FMiddle) AND ($P<$this->FB))  return ($this->FB-$P)/($this->FB-$this->FMiddle);
	 }

	 if ($this->FType==MemberShipFunctionType::TRAPEZOID->value) {
...
class Fuzzify extends Member {
	...
	public function addMember($idx,$Name='New',$start=0.0,$medium=0.0,$stop=0.0,$type=MemberShipFunctionType::TRIANGLE->value) {		
